...
$status = FALSE;
if ( authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["create"]) || 
authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["edit"]) || 
authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["view"]) || 
authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["delete"]) ) {
 $status = TRUE;
}
...
<div class="row">
    <div class="col-lg-9">
        ...
        <?php if (authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["create"])) { ?>
                ...
                <tbody><tr>
                    ...
                    <?php for ($i = 1; $i <= 6; $i++) { ?>
                        <tr>
                            ...
                            <td>1</td>
                            <td>
                                <?php if (authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["edit"])) { ?>
                                    <button class="btn btn-sm btn-info" type="button"><i class="fa fa-edit"></i> EDIT</button> 
                                <?php } ?>
                                <?php if (authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["view"])) { ?>
                                    <button class="btn btn-sm btn-warning" type="button"><i class="fa fa-search-plus"></i> VIEW</button>
                                <?php } ?>
                                <?php if (authorize($_SESSION["access"]["VALVES"]["TESTRESULTS"]["delete"])) { ?>
